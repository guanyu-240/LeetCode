...
public:
    ...
    void bfsSearch(string start, string end, unordered_set<string> &dict){
        ...
        while (!words.empty()) {
            ...
            for (int i = 0; i < tmp.length(); i ++) {
                ...
                for (char c = 'a'; c <= 'z'; c++) {
                    tmp[i] = c;
                    if (tmp == end) {
                        endSearch = true; predsMap[end].insert(cand);
                    }
                    if (!endSearch && dict.find(tmp) != dict.end()) {
                        cands.insert(tmp); predsMap[tmp].insert(cand);
                    }
                }
                tmp[i] = original;
            }
            if (words.empty()) {          // ready to start the next level or terminate
                if (endSearch) break;
                for (it = cands.begin(); it != cands.end(); ++ it) {
                    words.push(*it); dict.erase(*it);
                }
                cands.clear();
            }
        }
